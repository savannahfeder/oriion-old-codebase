import React from "react"
import TopBarPageSpecific from "../components/TopBarPageSpecific.js"
import Schedule from "../components/Schedule.js"


export default function ScheduleSelection() {
    return (
        <div className="schedule-selection">
            <TopBarPageSpecific page="Scheduling" />
            <div className="schedule-selection--container">
                <p className="bold schedule-selection--prompt">Daily Schedule</p>
                <p className="small-light schedule-selection--explanation">Evidence-backed explanation on the <strong>importance of taking this step.</strong> Also mention to add to calendar, and will determine when popups appear if enabled.</p>
                <Schedule />
            </div>
        </div>
    )
}